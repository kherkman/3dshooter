    ...
    levels: {
        city: {
            ...
            create: (scene, buildingColliders) => {
                ...
                const directionalLight = new THREE.DirectionalLight(0xffa500, 0.7); directionalLight.position.set(50, 50, 25); directionalLight.castShadow = true; directionalLight.shadow.mapSize.width = 1024; directionalLight.shadow.mapSize.height = 1024; directionalLight.shadow.camera.left = -100; directionalLight.shadow.camera.right = 100; directionalLight.shadow.camera.top = 100; directionalLight.shadow.camera.bottom = -100; scene.add(directionalLight);
                ...
                const wG = new THREE.BoxGeometry(0.8, 1.6, 0.1); 
                ...
                for (let i = 0; i < 200; i++) { 
                    ...
                    for (let y = wS * 2; y < h - wS; y += wS * 1.5) { 
                        for (let x = -w / 2 + wS; x < w / 2 - wS; x += wS) { 
                            if (Math.random() > 0.3) { 
                                const pos = new THREE.Vector3(p.x + x, y, p.y + d / 2 + 0.01);
                                const box = new THREE.Box3().setFromCenterAndSize(pos, new THREE.Vector3(0.8, 1.6, 0.1));
                                box.expandByScalar(0.2);
                                windowTransforms.push({ pos, rotY: 0, box, center: pos.clone() });
                            } 
                            if (Math.random() > 0.3) { 
                                const pos = new THREE.Vector3(p.x + x, y, p.y - d / 2 - 0.01);
                                const box = new THREE.Box3().setFromCenterAndSize(pos, new THREE.Vector3(0.8, 1.6, 0.1));
                                box.expandByScalar(0.2);
                                windowTransforms.push({ pos, rotY: Math.PI, box, center: pos.clone() });
                            } 
                        } 
                        for (let z = -d / 2 + wS; z < d / 2 - wS; z += wS) { 
                            if (Math.random() > 0.3) { 
                                const pos = new THREE.Vector3(p.x + w / 2 + 0.01, y, p.y + z);
                                const box = new THREE.Box3().setFromCenterAndSize(pos, new THREE.Vector3(0.1, 1.6, 0.8));
                                box.expandByScalar(0.2);
                                windowTransforms.push({ pos, rotY: Math.PI / 2, box, center: pos.clone() });
                            } 
                            if (Math.random() > 0.3) { 
                                const pos = new THREE.Vector3(p.x - w / 2 - 0.01, y, p.y + z);
                                const box = new THREE.Box3().setFromCenterAndSize(pos, new THREE.Vector3(0.1, 1.6, 0.8));
                ...
                if (windowTransforms.length > 0) {
                    const instancedWindows = new THREE.InstancedMesh(wG, wM, windowTransforms.length);
                    instancedWindows.receiveShadow = true;

                    const dummy = new THREE.Object3D();
                    const windowCenters = [];
                    const windowBoxes = [];
                    const windowRotations = [];

                    windowTransforms.forEach((wt, idx) => {
                        dummy.position.copy(wt.pos);
                        dummy.rotation.set(0, wt.rotY, 0);
                        dummy.scale.set(1, 1, 1);
                        dummy.updateMatrix();

                        instancedWindows.setMatrixAt(idx, dummy.matrix);

                        windowCenters.push(wt.center);
                        windowBoxes.push(wt.box);
                        windowRotations.push(wt.rotY);
                    });

                    instancedWindows.instanceMatrix.needsUpdate = true;

                    instancedWindows.userData = {
                        isWindowGroup: true,
            ...
            create: (scene, buildingColliders) => {
                ...
                const createCastle = (pos) => {
                    ...
                    [-1,1].forEach(sx => { [-1,1].forEach(sz => {
                        ...
                        if (sx === 1 && sz === 1) { 
                             designatedTowerTopPos = new THREE.Vector3(tPos.x, towerHeight + 1.5, tPos.z);
                        }
                    })});
        ...
        crystal: {
            ...
            create: (scene, buildingColliders) => {
                ...
                // PÄIVITETTY SUMUETÄISYYS: 220m, jotta 60m torni ja sen huipulla oleva kone näkyvät selkeästi
                scene.fog = new THREE.Fog(GameWorld.levels.crystal.fogColor, 0, 220);
                ...
                const crystalMat = new THREE.MeshStandardMaterial({
                    color: hasCrystalRockTex ? 0xffffff : 0xb388ff,
                    map: hasCrystalRockTex ? window.crystalRockTexture : null,
                    roughness: 0.2,
                    metalness: 0.4,
                    flatShading: false,
                    emissive: hasCrystalRockTex ? 0x4422aa : 0x7a33ff,
                    emissiveIntensity: hasCrystalRockTex ? 0.3 : 0.8,
                    transparent: true,
                    opacity: 0.85
                });
                ...
                const towerMat = new THREE.MeshStandardMaterial({
                    ...
                    map: hasTowerTex ? window.towerTexture : null,
                    roughness: 0.4,
                    metalness: 0.2
                });
                ...
                for (let i = 0; i < wallSegments; i++) {
                    ...
                    if (isDoorway) {
                        ...
                        const wallGeo = new THREE.BoxGeometry(segWidth, upperWallHeight, 1.6);
                        const wallMesh = new THREE.Mesh(wallGeo, towerMat);

                        wallMesh.position.set(x, doorwayHeight + upperWallHeight / 2, z);
                        ...
                        const wallGeo = new THREE.BoxGeometry(segWidth, towerHeight, 1.6);
                        const wallMesh = new THREE.Mesh(wallGeo, towerMat);

                        wallMesh.position.set(x, towerHeight / 2, z);
                        wallMesh.rotation.y = -angle + Math.PI / 2;
                        wallMesh.castShadow = true;
                        wallMesh.receiveShadow = true;
                ...
                // 2. Kerrostasanteet - LÄPÄISEMÄTTÖMÄT LATTIAELEMENTIT HISSIAUKOLLA (Säde 4.2m)
                ...
                for (let f = 1; f <= numFloors; f++) {
                    ...
                    floorShape.absarc(0, 0, towerRadius - 0.5, 0, Math.PI * 2, false);
                    ...
                    const floorMesh = new THREE.Mesh(floorExtrude, towerMat);
                ...
                // Lisätään kerrostasanteiden törmäyslaatikot ympäröimään keskiaukkoa
                const R = towerRadius - 0.5; // 13.5
                const r = holeRadius; // 4.2

                floorMeshes.forEach(({ mesh, fY }) => {
                    buildingColliders.push(mesh); // Mesh-törmäystarkistus sädehakua (raycasting) varten

                    const minY = towerPos.y + fY;
                    const maxY = towerPos.y + fY + 0.8;

                    // 4 törmäyslaatikkoa kerroksen ympärille siten, että keskireikä jää vapaaksi:
                    buildingColliders.push(new THREE.Box3(
                        new THREE.Vector3(towerPos.x - R, minY, towerPos.z - R),
                        new THREE.Vector3(towerPos.x + R, maxY, towerPos.z - r)
                    ));
                    buildingColliders.push(new THREE.Box3(
                        new THREE.Vector3(towerPos.x - R, minY, towerPos.z + r),
                        new THREE.Vector3(towerPos.x + R, maxY, towerPos.z + R)
                ...
                const elevatorMat = new THREE.MeshStandardMaterial({
                    color: hasElevatorTex ? 0xffffff : 0x3388ff,
                    map: hasElevatorTex ? window.crystalElevatorTexture : null,
                    metalness: 0.6,
                    roughness: 0.3,
                    emissive: hasElevatorTex ? 0x222244 : 0x0033aa,
                    emissiveIntensity: 0.3
                });
                const elevatorMesh = new THREE.Mesh(elevatorGeo, elevatorMat);
                elevatorMesh.position.set(towerPos.x, 0.25, towerPos.z);
                scene.add(elevatorMesh);
                elevatorMesh.updateMatrixWorld(true);

                const elevatorCollider = new THREE.Box3().setFromObject(elevatorMesh);
                elevatorCollider.userData = { isElevator: true, mesh: elevatorMesh };
                buildingColliders.push(elevatorCollider);
                ...
                brainRing1.rotation.x = Math.PI / 2;
                brainGroup.add(brainRing1);

                const brainRing2 = new THREE.Mesh(new THREE.TorusGeometry(4.8, 0.18, 16, 32), ringMat);
                brainRing2.position.y = towerHeight + 3.5;
                ...
                for (let i = 0; i < 12; i++) {
                    ...
                    spark.position.set(Math.cos(angle) * 3.6, towerHeight + 3.5 + Math.sin(angle * 2) * 0.8, Math.sin(angle) * 3.6);
                    brainGroup.add(spark);
                }

                brainGroup.position.copy(towerPos);
                ...
                topFuelCell.userData.key = 'fuel_cell';
                topFuelCell.userData.type = 'fuel_cell';
                topFuelCell.userData.fixedPosition = true;
                topFuelCell.userData.isFloatingAboveBrain = true;
                collectibles.fuelCells.push(topFuelCell);
                scene.add(topFuelCell);

                // Koneen törmäys- ja kestävyystiedot (Nostettu kestävyys 120 -> 300 osumaan!)
                ...
                return {
                    ...
                    towerFloors: floorPositions,
                    elevatorData: elevatorData,
                    crystalBrain: brainBox,
