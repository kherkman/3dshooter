...
function checkBuildingCollision(objectBox, isProjectile = true) {
    ...
    for (let i = buildingColliders.length - 1; i >= 0; i--) {
        ...
        // Nopeutettu tilallinen etäisyyssuodatus: Ohitetaan yli 35 m päässä olevat colliderit (35^2 = 1225)
        if (collider.isBox3) {
            collider.getCenter(tempColCenter);
            if (tempColCenter.distanceToSquared(tempObjCenter) > 1225.0) {
                continue;
            }
        } else if (collider.isMesh) {
            if (collider.position.distanceToSquared(tempObjCenter) > 1225.0) {
                continue;
            }
        }
