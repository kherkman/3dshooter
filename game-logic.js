 ...
 * handling Box3 colliders, Mesh colliders, and destructible props.
...
function checkBuildingCollision(objectBox, isProjectile = true) {
    for (let i = buildingColliders.length - 1; i >= 0; i--) {
        ...
        if (isHit) {
            // Tuhotaan roska-astiat ja katuvalot kun ammus osuu niihin
            if (isProjectile && collider.userData && collider.userData.isDestructible) {
 ...
 * triggers a shard debris effect, and drops contained items.
...
function updateCollectibles(delta) {
    ...
    // Handle single-instance items like jetpack and goggles (with mapped GameData itemKey)
