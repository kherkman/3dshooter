...
GameData.weapons = [
    {
        ...
        model: (isViewModel = true) => {
            ...
            // Pistoolin rungolle/bodyguard-osalle valittu pistolbody.jpg -materiaali
            const bodyMat = new THREE.MeshStandardMaterial({
                color: hasBodyTex ? 0xffffff : (hasMetalTex ? 0xffffff : 0x5a5048),
                map: hasBodyTex ? window.pistolBodyTexture : (hasMetalTex ? window.gunMetalTexture : null),
                metalness: 0.85,
                roughness: 0.38
            });
            ...
            // A. LUISTI - ETUOSA (Rungon muotoilussa mukana)
            const slideFrontGeo = new THREE.BoxGeometry(0.22, 0.26, 0.7);
            const slideFront = new THREE.Mesh(slideFrontGeo, bodyMat);
            ...
            const slideRear = new THREE.Mesh(slideRearGeo, shinySilverMat);
            ...
            // C. ISKURIN VASARA JA YKSITYISKOHDAT
            ...
            // D. TÄHTÄIMET (GREEN NIGHT SIGHTS)
            ...
            // F. RUNKO & ALAKISKO (Pääasiallinen pistolbody.jpg -kohde)
            ...
            // Tallennetut elementit rekyylianimaatiota varten
...
function shoot() {
    ...
    for (let i = 0; i < pellets; i++) {
        // Optimized: Uses cached geometry and material to prevent WebGL frame spikes
...
function ejectCasing() {
    // Optimized: Reuses shared geometries and materials for hylsy hienosäädöt
...
function updateBullets(delta) {
    for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        // Optimized: Mathematical Box3 bounding box instead of expensive setFromObject() vertex traversal
        const bulletBox = new THREE.Box3();
        bulletBox.min.copy(b.position).subScalar(0.1);
        bulletBox.max.copy(b.position).addScalar(0.1);
...
function updateRockets(delta) { 
    for (let i = rockets.length - 1; i >= 0; i--) { 
        ...
        // Optimized: Mathematical Box3 bounding box
        const rocketBox = new THREE.Box3();
        rocketBox.min.copy(r.position).subScalar(0.2);
        rocketBox.max.copy(r.position).addScalar(0.2);
...
function updatePlasmaRings(delta) {
    for (let i = plasmaRings.length - 1; i >= 0; i--) {
        ...
        ringBox.min.copy(ring.position).subScalar(0.6);
        ringBox.max.copy(ring.position).addScalar(0.6);
...
function updateGrenades(delta) { 
    for (let i = grenades.length - 1; i >= 0; i--) { 
        ...
        grenadeBox.min.copy(g.position).subScalar(0.2);
        grenadeBox.max.copy(g.position).addScalar(0.2);
...
function updateClusters(delta) { 
    for (let i = clusters.length - 1; i >= 0; i--) { 
        ...
        clusterBox.min.copy(c.position).subScalar(0.15);
        clusterBox.max.copy(c.position).addScalar(0.15);
...
function updateBlackHoles(delta) {
    for (let i = blackHoles.length - 1; i >= 0; i--) {
        ...
        if (bh.userData.state === 'projectile') {
            ...
            bhBox.min.copy(bh.position).subScalar(0.3);
            bhBox.max.copy(bh.position).addScalar(0.3);
