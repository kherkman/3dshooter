...
GameData.weapons = [
    {
        ...
        model: (isViewModel = true) => {
            ...
            const darkMetal = new THREE.MeshStandardMaterial({ 
                color: hasMetalTex ? 0xffffff : 0x4a4a4a, 
                map: hasMetalTex ? window.gunMetalTexture : null,
                roughness: 0.3, 
                metalness: 0.9 
            }); 
            const gripMat = new THREE.MeshStandardMaterial({ 
                color: hasWoodTex ? 0xffffff : 0x633a21, 
                map: hasWoodTex ? window.gunWoodTexture : null,
                roughness: 0.7 
            }); 
            const grip = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.35, 0.2), gripMat); 
            grip.position.set(0, -0.15, 0.05); 
            grip.rotation.x = -0.2; 
            group.add(grip); 
            const frame = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.18, 0.3), darkMetal); 
            frame.position.set(0, 0.08, 0); 
            group.add(frame); 
            const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.4, 16), darkMetal); 
            barrel.rotation.x = Math.PI / 2; 
            barrel.position.set(0, 0.15, -0.35); 
            group.add(barrel); 
            const cylinder = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.09, 0.25, 6), darkMetal); 
            cylinder.rotation.x = Math.PI/2; 
            cylinder.position.set(0, 0.03, 0); 
            group.add(cylinder); 
            const hammer = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.08, 0.05), darkMetal); 
            hammer.position.set(0, 0.18, 0.15); 
            group.add(hammer); 
            group.userData.cylinder = cylinder; 
