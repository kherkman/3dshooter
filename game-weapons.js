...
GameData.weapons = [
    {
        ...
        model: (isViewModel = true) => {
            ...
            // Rungon materiaali: Hieman tummempi harmaa kuin luisti
            ...
            // Taktiset osat (vaimennin, yötähtäinten runko, urat)
...
function updateBullets(delta) {
    for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        // Track trajectory using previous position
        const prevPos = b.position.clone();
        b.position.add(b.userData.velocity.clone().multiplyScalar(delta));
        let hit = false;

        // Trajectory-based Box3 bounding box (Swept Box) prevents bullet tunneling through thin windows/walls
