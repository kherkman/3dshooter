...
GameData.weapons = [
    {
        ...
        model: (isViewModel = true) => {
            ...
            const hasMetalTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.gunMetalTexture;
            const hasBodyTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.pistolBodyTexture;
            ...
            const slideMat = new THREE.MeshStandardMaterial({
                ...
                metalness: 0.6,
                roughness: 0.25,
                emissive: hasBodyTex ? 0x222222 : 0x000000
            });
            ...
            const frameMat = new THREE.MeshStandardMaterial({
                color: hasMetalTex ? 0xffffff : 0x666666,
                map: hasMetalTex ? window.gunMetalTexture : null,
                metalness: 0.8,
                roughness: 0.35
            });
            ...
            const darkTacticalMat = new THREE.MeshStandardMaterial({
                color: hasMetalTex ? 0xdddddd : 0x555555,
                map: hasMetalTex ? window.gunMetalTexture : null,
                metalness: 0.8,
                roughness: 0.4
            });
            ...
            const rubberGripMat = new THREE.MeshStandardMaterial({
                color: hasMetalTex ? 0xcccccc : 0x444444,
                map: hasMetalTex ? window.gunMetalTexture : null,
                metalness: 0.3,
                roughness: 0.6
            });
    ...
    },
    {
        ...
        model: (isViewModel = true) => {
            const group = new THREE.Group();
            const hasMetalTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.gunMetalTexture;
            const hasWoodTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.gunWoodTexture;
    ...
    },
    {
        ...
        model: (isViewModel = true) => {
            ...
            const greenMetal = new THREE.MeshStandardMaterial({ 
                color: hasMetalTex ? 0xffffff : 0x3d553d, 
                map: hasMetalTex ? window.gunMetalTexture : null,
                roughness: 0.6, 
                metalness: 0.8 
            }); 
            const darkMetal = new THREE.MeshStandardMaterial({ 
                color: hasMetalTex ? 0xdddddd : 0x222222, 
                map: hasMetalTex ? window.gunMetalTexture : null,
                roughness: 0.5, 
                metalness: 0.9 
            }); 
    ...
    },
    {
        ...
        model: (isViewModel = true) => {
            ...
            const bodyMat = new THREE.MeshStandardMaterial({ 
                color: hasMetalTex ? 0xffffff : 0x2a2a3a, 
                map: hasMetalTex ? window.gunMetalTexture : null,
                roughness: 0.4, 
                metalness: 0.9 
            }); 
    ...
    },
    {
        ...
        model: (isViewModel = true) => {
            ...
            const darkMetal = new THREE.MeshStandardMaterial({ 
                color: hasMetalTex ? 0xffffff : 0x333333, 
                map: hasMetalTex ? window.gunMetalTexture : null,
                roughness: 0.5, 
                metalness: 1.0 
            }); 
    ...
    {
        name: 'Battle Axe',
        description: 'A brutal melee weapon. Can chop through alien vegetation and vehicles.',
        levels: ['city', 'desert', 'volcanic', 'ice', 'toxic', 'crystal'],
        properties: { fireRate: 1.2, ammoType: 'axe', range: 6.0, sound: 'gun_axe' },
        model: (isViewModel = true) => {
            const group = new THREE.Group();
            const hasMetalTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.gunMetalTexture;
            const hasWoodTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.gunWoodTexture;
            ...
            switch(anim.state) {
                ...
                case 'swinging_up': {
                    ...
                    if (!anim.hasHit && alpha > 0.1 && alpha < 0.95) {
                        ...
                        if (playerObject) {
                            pForward.applyQuaternion(playerObject.quaternion);
                        } else if (typeof camera !== 'undefined') {
                            camera.getWorldDirection(pForward);
                        }
                        pForward.y = 0;
                        pForward.normalize();
                        ...
                        for (let j = aliens.length - 1; j >= 0; j--) {
                            ...
                            const dx = a.position.x - pPos.x;
                            const dz = a.position.z - pPos.z;
                            const dy = Math.abs((a.position.y + 0.8) - (pPos.y + 0.9));
                            const distXZ = Math.sqrt(dx * dx + dz * dz);

                            if (distXZ <= meleeReach && dy <= 4.0) {
                                const dirX = distXZ > 0.0001 ? dx / distXZ : 0;
                                const dirZ = distXZ > 0.0001 ? dz / distXZ : 0;
                                const dotXZ = pForward.x * dirX + pForward.z * dirZ;

                                if (dotXZ > -0.1 || distXZ < 2.8) {
                                    a.userData.health -= 10;

                                    const enemyCenter = a.position.clone().add(new THREE.Vector3(0, 1.0, 0));
                                    if (typeof createHitScatter === 'function') {
                                        createHitScatter(enemyCenter, 0xff0000);
                                    }
                                    ...
                                    if (a.userData.health <= 0) {
                                        ...
                                        if (a.userData.type === 'shard_roller') {
                                            if (typeof spawnShardMites === 'function') spawnShardMites(a.position);
                                            scene.remove(a);
                                            aliens.splice(j, 1);
                                        } else if (a.userData.type === 'dome_guardian') {
                                            if (typeof createAlienDebris === 'function') createAlienDebris(a.position, 0xffff00);
                                            scene.remove(a);
                                            aliens.splice(j, 1);
                                        } else {
                                            const deathSound = a.userData.type === 'cyborg' ? 'cyborg_death' : 'alien_death';
                                            if (typeof playSound === 'function') playSound(deathSound);
                                            if (a.userData.type === 'cyborg') {
                                                ...
                                                if (typeof createAlienDebris === 'function') createAlienDebris(a.position, color);
                        ...
                        if (typeof buildingColliders !== 'undefined') {
                            ...
                            for (let c of buildingColliders) {
                                if (c.userData && c.userData.isCrystalBrain && !c.userData.isDestroyed) {
                                    ...
                                    if (brainCenter.distanceTo(axeHitPos) < 6.0) {
                                        if (typeof damageCrystalBrain === 'function') {
                                            damageCrystalBrain(c, brainCenter, 40);
                                        }
                                        anim.hasHit = true;
                                        break;
                                    }
    ...
    },
    {
        ...
        model: (isViewModel = true) => {
            ...
            const bodyMat = new THREE.MeshStandardMaterial({ 
                color: hasMetalTex ? 0xffffff : 0x22252a, 
                map: hasMetalTex ? window.gunMetalTexture : null,
                roughness: 0.4, 
                metalness: 0.9 
            });
            const scopeMat = new THREE.MeshStandardMaterial({ 
                color: hasMetalTex ? 0xdddddd : 0x111111, 
                map: hasMetalTex ? window.gunMetalTexture : null,
                roughness: 0.3, 
                metalness: 1.0 
            });
    ...
    },
    {
        ...
        model: (isViewModel = true) => {
            ...
            const bodyMat = new THREE.MeshStandardMaterial({
                color: hasMetalTex ? 0xffffff : 0x1a1a2a, 
                map: hasMetalTex ? window.gunMetalTexture : null,
                metalness: 1.0, 
                roughness: 0.2
            });
...
function updateBullets(delta) {
    for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        // 1. TARKISTETAAN ENSIN VIHOLLISET
        for (let j = aliens.length - 1; j >= 0; j--) {
            const a = aliens[j];
            if (a.userData.state === 'dying') continue;
        ...
        // 2. TARKISTETAAN SEINÄT/MAA/AUTOT VAIN JOS EI OSUTTU VIHOLLISEEN
        if (!hit) {
            const bulletBox = new THREE.Box3();
            bulletBox.setFromPoints([prevPos, b.position]);
            bulletBox.expandByScalar(0.15);

            // Määritetään luodin asekohdainen vahinko
            let bulletDamage = 10;
            if (b.userData.weaponType === 'Pistol') bulletDamage = 12;
            else if (b.userData.weaponType === 'Shotgun') bulletDamage = 10;
            else if (b.userData.weaponType === 'Machine Gun') bulletDamage = 8;
            else if (b.userData.weaponType === 'Sniper Rifle') bulletDamage = 60;

            if (checkBuildingCollision(bulletBox, true, bulletDamage) || b.position.y <= 0) {
                hit = true; 
...
function updateRockets(delta) { 
    for (let i = rockets.length - 1; i >= 0; i--) { 
        ...
        if (!hit && checkBuildingCollision(rocketBox, true, 100)) { 
            hit = true; 
        } 
...
function updatePlasmaRings(delta) {
    for (let i = plasmaRings.length - 1; i >= 0; i--) {
        ...
        if (checkBuildingCollision(ringBox, true, 25)) {
...
function updateGrenades(delta) { 
    for (let i = grenades.length - 1; i >= 0; i--) { 
        ...
        if (checkBuildingCollision(grenadeBox, true, 60) || g.position.y <= 0.2) { 
            hit = true; 
        } 
        if (!hit) { 
            for (let j = aliens.length - 1; j >= 0; j--) { 
...
function updateClusters(delta) { 
    for (let i = clusters.length - 1; i >= 0; i--) { 
        ...
        if (checkBuildingCollision(clusterBox, true, 25) || c.position.y <= 0.1) { 
            hit = true; 
        } 
        if (!hit) { 
            for (let j = aliens.length - 1; j >= 0; j--) { 
...
function updateBlackHoles(delta) {
    for (let i = blackHoles.length - 1; i >= 0; i--) {
        ...
        if (bh.userData.state === 'projectile') {
            ...
            let hit = false;
            if (checkBuildingCollision(bhBox, true, 150) || bh.position.y <= 0.2) {
                hit = true;
            }
