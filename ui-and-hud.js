...
function updateHUD() {
    ...
    // Cache the weapon sprites query on demand when HUD updates
    ...
    if (player.hasJetpack && jetpackBar) {
        jetpackBar.style.width = `${(player.jetpackFuel / player.maxJetpackFuel) * 100}%`;
    }

    if (player.hasXRayGoggles && gogglesContainer && gogglesBar) {
...
function createMapBackground() {
    ...
    buildingColliders.forEach(c => {
        // Jos kolliideri on Mesh-tyyppinen (esim. Volcanic-tason pyramidi), luodaan tilapäinen Box3-ympärystä kartan mittasuhteita varten
        let box = c;
        if (c.isMesh) {
...
function updateMap(ctx = mapCtx, width = mapCanvas.width, height = mapCanvas.height) {
    ...
    // Optimized: Reuses calculation variables mathematically instead of spawning functions inside frame loops
