...
function updateMap(ctx = mapCtx, width = mapCanvas.width, height = mapCanvas.height) {
    ...
    // Avaruusaluksen piirtäminen kartalle VIHREÄNÄ (#00ff00)
    if (spacecraft && spacecraft.parent) {
        const scMapX = (spacecraft.position.x + halfSize) * scaleX;
        const scMapY = (spacecraft.position.z + halfSize) * scaleY;
        ctx.fillStyle = '#00ff00';
        ctx.beginPath();
        ctx.arc(scMapX, scMapY, 5, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = '#ffffff';
        ctx.lineWidth = 1;
        ctx.stroke();
    ...
    // Crystal-kentän TORNIN piirtäminen kartalle PUNAISELLA (#ff0000)
    if (currentLevel === 'crystal' && levelObjects.towerPos) {
        const twMapX = (levelObjects.towerPos.x + halfSize) * scaleX;
        const twMapY = (levelObjects.towerPos.z + halfSize) * scaleY;
        const twRadius = Math.max(6, (levelObjects.towerRadius || 14) * scaleX);
        ctx.fillStyle = '#ff0000';
        ctx.beginPath();
        ctx.arc(twMapX, twMapY, twRadius, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = '#ffffff';
        ctx.lineWidth = 1.5;
        ctx.stroke();
