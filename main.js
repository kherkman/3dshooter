...
function loadLevel(levelName, isInitialLoad = false, isLanding = false) {
    ...
    // Dynamic city window identification and precomputed bounding box caching with optimized scale
    cityWindows.length = 0;
    if (levelName === 'city') {
        scene.traverse(child => {
            if (child.isMesh && child.material && child.material.emissive && child.material.emissive.getHex() === 0xffaa00) {
                ...
                box.expandByScalar(0.2); // Pienennetty 0.6 -> 0.2 (tarkempi osuma, mutta edelleen helppo tuhota)
...
function spawnSmokeParticle(emitterPos) {
    const geo = new THREE.SphereGeometry(Math.random() * 0.15 + 0.1, 8, 8);
    ...
    smokeParticles.push({ mesh: p, vel: vel, life: 2.0, maxLife: 2.0 });
    scene.add(p);
}
...
function animate() {
    ...
    if (!isPaused) {
        ...
        // --- MOOTTORISAVUN PÄIVITYS TUHOUTUNEISTA AUTOISTA ---
        cityCars.forEach(car => {
...
// Override checkBuildingCollision to handle preemptive window hits before wall absorption with distance filter
...
checkBuildingCollision = function(objectBox, isProjectile = true) {
    if (currentLevel === 'city' && cityWindows.length > 0 && isProjectile) {
        ...
        for (let i = 0; i < cityWindows.length; i++) {
            ...
            // Etäisyyssuodatus suorituskyvyn parantamiseksi (ohitetaan yli 5 metrin päässä olevat ikkunat)
            if (win.userData.center && win.userData.center.distanceToSquared(objCenter) > 25.0) {
                continue;
            }

            const windowBox = win.userData.expandedBox;
