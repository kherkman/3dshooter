...
function init() {
    ...
    // Initialize Texture Loader and Load Textures with immediate global assignment
    ...
    // Load Weapon and Enemy Textures
    ...
    // Optimized shared assets for smoke particles to prevent GC pauses
    ...
    // WebAudio Autoplay bypass, Fullscreen request & Mobile HTML5 Audio unlocking
    const startIntroMusicOnInteraction = () => {
        if (!hasInteracted) {
            ...
            // Trigger Fullscreen immediately on initial screen interaction
...
function advanceIntro() {
    ...
    // Force Fullscreen mode on start/advance
    if (!document.fullscreenElement && document.documentElement.requestFullscreen) {
...
function onWindowResize() {
    ...
    // Keep HUD fitted strictly across the full horizontal screen width
...
function loadLevel(levelName, isInitialLoad = false, isLanding = false) {
    ...
    // Spawn Spacecraft and Vehicles BEFORE Initial Collectibles
    ...
    // Dynamic city window identification and precomputed bounding box caching with spatial optimization
    cityWindows.length = 0;
    if (levelName === 'city') {
        scene.traverse(child => {
            if (child.isInstancedMesh && child.userData && child.userData.isWindowGroup) {
                // InstancedMesh-ikkunaryhmät (Optimoitu tapa)
...
function animate() {
    ...
    if (!isPaused) {
        ...
        // --- HOVERBIKE SOUND 1-SHOT CONTROL (NO LOOPING) ---
        ...
        // --- MOOTTORISAVUN PÄIVITYS TUHOUTUNEISTA AUTOISTA & SAVUAVASTA FUEL CELL ROSKA-ASTIASTA ---
        cityCars.forEach(car => {
        ...
        // Stop hoverbike sound immediately when paused
        if (hoverbikeSoundActive || hoverbikeSound) {
            hoverbikeSoundActive = false;
            if (hoverbikeSound) {
                try { if (hoverbikeSound.isPlaying) hoverbikeSound.stop(); } catch(e) {}
...
// Override checkBuildingCollision to handle preemptive window hits before wall absorption with distance filter & InstancedMesh support
...
checkBuildingCollision = function(objectBox, isProjectile = true) {
    if (currentLevel === 'city' && cityWindows.length > 0 && isProjectile) {
        ...
        for (let i = 0; i < cityWindows.length; i++) {
            ...
            // Spatial culling: skip checking windows further than 5 meters (squared dist > 25)
...
function breakWindow(windowRef, hitPosition) {
    ...
    if (windowRef.isInstanced) {
        // InstancedMesh-ikkunan piilottaminen
        ...
        // Luodaan yksittäinen särkynyt ikkuna-mesh täsmälleen osuneen ikkunan paikalle
