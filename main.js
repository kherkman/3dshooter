...
// --- JETPACK, HOVERBIKE & SPACECRAFT SOUNDS STATE ---
let jetpackSound = null;
let jetpackSoundActive = false;
let hoverbikeSound = null;
let hoverbikeSoundActive = false;
...
const cityWindows = []; // Cache for city level window meshes/instances to optimize collision checking
const cityCars = []; // Cache for destructible cars in city level
const smokeParticles = []; // Array for engine smoke particles
const fuelCellPulseSpheres = []; // Array for expanding blue goggles pulse spheres around fuel cells
...
let sharedShardGeometry, sharedShardMaterial;
let sharedDebrisGeometry;
let sharedSmokeGeometry, sharedSmokeMaterial;
...
let gunWoodTexture, gunMetalTexture, pistolBodyTexture;
...
let trashTexture, streetLightTexture; // Prop textures
let carTexture, carDestroyedTexture; // Car textures
let towerTexture, crystalBrainTexture, crystalElevatorTexture; // Tornin, koneen ja hissin tekstit

// Collectible Item Textures
...
// --- SEINÄTURVALLINEN TÖNÄISYFUNKTIO VIHOLLISTEN TÖMÄYKSILLE ---
function pushPlayerSafely(pushX, pushZ) {
    ...
    if (Math.abs(pushX) > 0.0001) {
        ...
        if (!checkBuildingCollision(testBox, false)) {
            playerObject.position.x += pushX;
        } else {
            player.velocity.x = 0;
        }
    }
    ...
    if (Math.abs(pushZ) > 0.0001) {
        ...
        if (!checkBuildingCollision(testBox, false)) {
            playerObject.position.z += pushZ;
        } else {
            player.velocity.z = 0;
        }
    }
}
window.pushPlayerSafely = pushPlayerSafely;
...
function init() {
    ...
    window.cityWallTexture = cityWallTexture = textureLoader.load('wall.jpg');
    window.cityWindowTexture = cityWindowTexture = textureLoader.load('window.jpg');
    window.cityWindowBrokenTexture = cityWindowBrokenTexture = textureLoader.load('windowbroken.jpg');
    window.cityFloorTexture = cityFloorTexture = textureLoader.load('cityfloor.jpg');
    window.desertRockTexture = desertRockTexture = textureLoader.load('desertrock.jpg');
    window.toxicFloorTexture = toxicFloorTexture = textureLoader.load('toxicfloor.jpg');
    window.toxicStemTexture = toxicStemTexture = textureLoader.load('toxicstem.jpg');
    window.toxicLeafTexture = toxicLeafTexture = textureLoader.load('toxicleaf.jpg');

    window.iceRockTexture = iceRockTexture = textureLoader.load('icerock.jpg');
    window.iceFloorTexture = iceFloorTexture = textureLoader.load('icefloor.jpg');
    window.castleTexture = castleTexture = textureLoader.load('castle.jpg');
    window.crystalRockTexture = crystalRockTexture = textureLoader.load('crystalrock.jpg');
    window.crystalFloorTexture = crystalFloorTexture = textureLoader.load('crystalfloor.jpg');
    window.pyramidTexture = pyramidTexture = textureLoader.load('pyramid.jpg');
    window.volcanicFloorTexture = volcanicFloorTexture = textureLoader.load('volcanicfloor.jpg');
    window.volcanicRockTexture = volcanicRockTexture = textureLoader.load('volcanicrock.jpg');
    window.desertFloorTexture = desertFloorTexture = textureLoader.load('desertfloor.jpg');
    window.spacecraftTexture = spacecraftTexture = textureLoader.load('spacecraft.jpg');
    window.hoverbikeTexture = hoverbikeTexture = textureLoader.load('hoverbike.jpg');

    window.trashTexture = trashTexture = textureLoader.load('trash.jpg');
    window.trashCanTexture = trashTexture;
    window.streetLightTexture = streetLightTexture = textureLoader.load('streetlight.jpg');
    window.carTexture = carTexture = textureLoader.load('car.jpg');
    window.carDestroyedTexture = carDestroyedTexture = textureLoader.load('car-destroyed.jpg', undefined, undefined, () => {
        window.carDestroyedTexture = carDestroyedTexture = textureLoader.load('cardestroyed.jpg');
    });

    window.towerTexture = towerTexture = textureLoader.load('tower.jpg');
    window.crystalBrainTexture = crystalBrainTexture = textureLoader.load('crystalbrain.jpg');
    window.crystalElevatorTexture = crystalElevatorTexture = textureLoader.load('crystalelevator.jpg');

    window.gunWoodTexture = gunWoodTexture = textureLoader.load('gunwood.jpg', onWeaponTextureLoad);
    window.gunMetalTexture = gunMetalTexture = textureLoader.load('gunmetal.jpg', onWeaponTextureLoad);
    window.pistolBodyTexture = pistolBodyTexture = textureLoader.load('pistolbody.jpg', onWeaponTextureLoad);
    window.shardRollerTexture = shardRollerTexture = textureLoader.load('shardroller.jpg');
    window.shardMiteTexture = shardMiteTexture = textureLoader.load('shardmite.jpg');
    window.stalkerMouthTexture = stalkerMouthTexture = textureLoader.load('stalker.jpg');
    window.domeGuardianTexture = domeGuardianTexture = textureLoader.load('domeguardian.jpg');
    window.enemyBlackTexture = enemyBlackTexture = textureLoader.load('enemyblack.jpg');
    window.crystalWallTexture = crystalWallTexture = textureLoader.load('crystalwall.jpg');
    window.crystalBlockTexture = crystalBlockTexture = textureLoader.load('crystalblock.jpg');
    
    window.fuelCellTexture = fuelCellTexture = textureLoader.load('fuelcell.jpg');
    window.jetpackTexture = jetpackTexture = textureLoader.load('jetpack.jpg');
    window.gogglesTexture = gogglesTexture = textureLoader.load('goggles.jpg');
    window.healthTexture = healthTexture = textureLoader.load('health.jpg');
    window.glowingOrbTexture = glowingOrbTexture = textureLoader.load('glowingorb.jpg');

    cityWallTexture.wrapS = THREE.RepeatWrapping; cityWallTexture.wrapT = THREE.RepeatWrapping;
    cityFloorTexture.wrapS = THREE.RepeatWrapping; cityFloorTexture.wrapT = THREE.RepeatWrapping;
    desertRockTexture.wrapS = THREE.RepeatWrapping; desertRockTexture.wrapT = THREE.RepeatWrapping;
    toxicFloorTexture.wrapS = THREE.RepeatWrapping; toxicFloorTexture.wrapT = THREE.RepeatWrapping;
    toxicStemTexture.wrapS = THREE.RepeatWrapping; toxicStemTexture.wrapT = THREE.RepeatWrapping;
    toxicLeafTexture.wrapS = THREE.RepeatWrapping; toxicLeafTexture.wrapT = THREE.RepeatWrapping;

    iceRockTexture.wrapS = THREE.RepeatWrapping; iceRockTexture.wrapT = THREE.RepeatWrapping;
    iceFloorTexture.wrapS = THREE.RepeatWrapping; iceFloorTexture.wrapT = THREE.RepeatWrapping;
    castleTexture.wrapS = THREE.RepeatWrapping; castleTexture.wrapT = THREE.RepeatWrapping;
    crystalRockTexture.wrapS = THREE.RepeatWrapping; crystalRockTexture.wrapT = THREE.RepeatWrapping;
    crystalFloorTexture.wrapS = THREE.RepeatWrapping; crystalFloorTexture.wrapT = THREE.RepeatWrapping;
    pyramidTexture.wrapS = THREE.RepeatWrapping; pyramidTexture.wrapT = THREE.RepeatWrapping;
    volcanicFloorTexture.wrapS = THREE.RepeatWrapping; volcanicFloorTexture.wrapT = THREE.RepeatWrapping;
    volcanicRockTexture.wrapS = THREE.RepeatWrapping; volcanicRockTexture.wrapT = THREE.RepeatWrapping;
    desertFloorTexture.wrapS = THREE.RepeatWrapping; desertFloorTexture.wrapT = THREE.RepeatWrapping;
    spacecraftTexture.wrapS = THREE.RepeatWrapping; spacecraftTexture.wrapT = THREE.RepeatWrapping;
    hoverbikeTexture.wrapS = THREE.RepeatWrapping; hoverbikeTexture.wrapT = THREE.RepeatWrapping;

    if (towerTexture) { towerTexture.wrapS = THREE.RepeatWrapping; towerTexture.wrapT = THREE.RepeatWrapping; }
    if (crystalBrainTexture) { crystalBrainTexture.wrapS = THREE.RepeatWrapping; crystalBrainTexture.wrapT = THREE.RepeatWrapping; }
    if (crystalElevatorTexture) { crystalElevatorTexture.wrapS = THREE.RepeatWrapping; crystalElevatorTexture.wrapT = THREE.RepeatWrapping; }

    trashTexture.wrapS = THREE.RepeatWrapping; trashTexture.wrapT = THREE.RepeatWrapping;
    streetLightTexture.wrapS = THREE.RepeatWrapping; streetLightTexture.wrapT = THREE.RepeatWrapping;
    if (carTexture) { carTexture.wrapS = THREE.RepeatWrapping; carTexture.wrapT = THREE.RepeatWrapping; }
    if (carDestroyedTexture) { carDestroyedTexture.wrapS = THREE.RepeatWrapping; carDestroyedTexture.wrapT = THREE.RepeatWrapping; }

    gunWoodTexture.wrapS = THREE.RepeatWrapping; gunWoodTexture.wrapT = THREE.RepeatWrapping;
    gunMetalTexture.wrapS = THREE.RepeatWrapping; gunMetalTexture.wrapT = THREE.RepeatWrapping;
    pistolBodyTexture.wrapS = THREE.RepeatWrapping; pistolBodyTexture.wrapT = THREE.RepeatWrapping;
    shardRollerTexture.wrapS = THREE.RepeatWrapping; shardRollerTexture.wrapT = THREE.RepeatWrapping;
    shardMiteTexture.wrapS = THREE.RepeatWrapping; shardMiteTexture.wrapT = THREE.RepeatWrapping;
    stalkerMouthTexture.wrapS = THREE.RepeatWrapping; stalkerMouthTexture.wrapT = THREE.RepeatWrapping;
    domeGuardianTexture.wrapS = THREE.RepeatWrapping; domeGuardianTexture.wrapT = THREE.RepeatWrapping;
    enemyBlackTexture.wrapS = THREE.RepeatWrapping; enemyBlackTexture.wrapT = THREE.RepeatWrapping;
    crystalWallTexture.wrapS = THREE.RepeatWrapping; crystalWallTexture.wrapT = THREE.RepeatWrapping;
    crystalBlockTexture.wrapS = THREE.RepeatWrapping; crystalBlockTexture.wrapT = THREE.RepeatWrapping;

    fuelCellTexture.wrapS = THREE.RepeatWrapping; fuelCellTexture.wrapT = THREE.RepeatWrapping;
    jetpackTexture.wrapS = THREE.RepeatWrapping; jetpackTexture.wrapT = THREE.RepeatWrapping;
    gogglesTexture.wrapS = THREE.RepeatWrapping; gogglesTexture.wrapT = THREE.RepeatWrapping;
    healthTexture.wrapS = THREE.RepeatWrapping; healthTexture.wrapT = THREE.RepeatWrapping;
    glowingOrbTexture.wrapS = THREE.RepeatWrapping; glowingOrbTexture.wrapT = THREE.RepeatWrapping;

    window.cityCars = cityCars;
    window.smokeParticles = smokeParticles;
    window.spawnSmokeParticle = spawnSmokeParticle;

    sharedShardGeometry = new THREE.ConeGeometry(0.06, 0.18, 3);
    sharedShardMaterial = new THREE.MeshStandardMaterial({
        color: 0xddf0ff,
        transparent: true,
        opacity: 0.85,
        roughness: 0.1,
    ...
    sharedDebrisGeometry = new THREE.SphereGeometry(0.06, 8, 8);

    sharedSmokeGeometry = new THREE.SphereGeometry(0.2, 8, 8);
    sharedSmokeMaterial = new THREE.MeshBasicMaterial({
        color: 0x1f1f1f,
        transparent: true,
        opacity: 0.65
    });
    ...
    const startIntroMusicOnInteraction = () => {
        if (!hasInteracted) {
            ...
            if (!document.fullscreenElement && document.documentElement.requestFullscreen) {
                document.documentElement.requestFullscreen().catch(e => console.warn("Fullscreen request on interaction:", e));
            }
    ...
    if (isTouchDevice) {
        let warningDiv = document.getElementById('portrait-warning');
        if (!warningDiv) {
...
function advanceIntro() {
    ...
    if (!document.fullscreenElement && document.documentElement.requestFullscreen) {
        document.documentElement.requestFullscreen().catch(e => console.warn("Fullscreen request in advanceIntro:", e));
    }
...
function onWindowResize() {
    ...
    if (isTouchDevice && warning) {
        if (h > w) {
            ...
            if (!isIntroActive && !isGameOver && !document.getElementById('options-menu')?.style.display.includes('flex')) {
                isPaused = false;
            }
        }
    }
    ...
    if (doomHud) {
        doomHud.style.width = '100vw';
        doomHud.style.left = '0px';
        doomHud.style.bottom = '0px';
    }
...
function clearScene() {
    ...
    fuelCellPulseSpheres.forEach(p => {
        if (p.geometry) p.geometry.dispose();
        if (p.material) p.material.dispose();
        scene.remove(p);
    });
    fuelCellPulseSpheres.length = 0;
...
function loadLevel(levelName, isInitialLoad = false, isLanding = false) {
    ...
    if (levelName === 'city') {
        scene.traverse(child => {
            if (child.isInstancedMesh && child.userData && child.userData.isWindowGroup) {
...
function spawnSmokeParticle(emitterPos) {
    ...
    const p = new THREE.Mesh(sharedSmokeGeometry, sharedSmokeMaterial.clone());
    p.position.copy(emitterPos).add(new THREE.Vector3(
        (Math.random() - 0.5) * 0.4,
        0,
        (Math.random() - 0.5) * 0.4
    ));
    ...
    const baseScale = Math.random() * 0.75 + 0.5;
    p.scale.setScalar(baseScale);

    smokeParticles.push({ mesh: p, vel: vel, life: 2.0, maxLife: 2.0, baseScale: baseScale });
    scene.add(p);
}

function updateFuelCellPulseSpheres(delta) {
    if (!player.xrayGogglesActive) {
        if (fuelCellPulseSpheres.length > 0) {
            fuelCellPulseSpheres.forEach(p => scene.remove(p));
            fuelCellPulseSpheres.length = 0;
        }
        return;
    }
    ...
    const targets = [];
    collectibles.fuelCells.forEach(fc => {
        if (fc && fc.parent) targets.push(fc.position);
    });
    buildingColliders.forEach(c => {
        if (c.userData && c.userData.containsItem === 'fuel_cell' && !c.userData.isDestroyed && c.userData.mesh) {
            targets.push(c.userData.mesh.position);
        }
    });

    while (fuelCellPulseSpheres.length < targets.length) {
        ...
        scene.add(mesh);
        fuelCellPulseSpheres.push(mesh);
    }

    while (fuelCellPulseSpheres.length > targets.length) {
        const mesh = fuelCellPulseSpheres.pop();
        scene.remove(mesh);
    ...
    const currentScale = 0.5 + cycle * 4.0;
    const currentOpacity = (1.0 - cycle) * 0.7;

    for (let i = 0; i < targets.length; i++) {
        const sphere = fuelCellPulseSpheres[i];
        sphere.position.copy(targets[i]).add(new THREE.Vector3(0, 0.5, 0));
        sphere.scale.setScalar(currentScale);
        sphere.material.opacity = currentOpacity;
    }
}
...
function updateElevator(delta) {
    ...
    if (elData.mesh.position.y >= elData.maxY) {
        elData.mesh.position.y = elData.maxY;
        elData.direction = -1;
    ...
    if (player.state === 'on_foot' && playerObject) {
        ...
        const playerFeetY = playerObject.position.y - GameWorld.player.height / 2;
        const elevatorTopY = elData.mesh.position.y + 0.25;

        if (distSq < 16.0 && Math.abs(playerFeetY - elevatorTopY) < 0.6) {
            playerObject.position.y += actualDeltaY;
            player.canJump = true;
            player.velocity.y = 0;
        }
    }
...
function animate() {
    ...
    updateFuelCellPulseSpheres(delta);
    ...
    if (!isPaused) {
        ...
        const isTouchMovingBike = Object.values(touchState).some(s => s.zone === 'move');
        const isHoverbikeMoving = player.state === 'driving_motorcycle' && (keys['KeyW'] || keys['KeyS'] || isTouchMovingBike) && !isPaused;

        if (isHoverbikeMoving) {
            if (!hoverbikeSoundActive) {
                hoverbikeSoundActive = true;
                if (gameSounds['hoverbike']) {
                    hoverbikeSound = new THREE.Audio(audioListener);
                    hoverbikeSound.setBuffer(gameSounds['hoverbike']);
                    hoverbikeSound.setLoop(false);
                    hoverbikeSound.setVolume(gameSettings.sfxVolume);
                    hoverbikeSound.play();
                }
            }
        } else {
            if (hoverbikeSoundActive || hoverbikeSound) {
                ...
                if (hoverbikeSound) {
                    try { if (hoverbikeSound.isPlaying) hoverbikeSound.stop(); } catch(e) {}
                    hoverbikeSound = null;
                }
            }
        }
        ...
        cityCars.forEach(car => {
            if (car.state && car.state.frontDestroyed) {
                if (Math.random() < 0.4) {
                    const smokePos = car.mesh.localToWorld(new THREE.Vector3(0, 0.58 * 2, 1.15 * 2));
        ...
        buildingColliders.forEach(c => {
            if (c.userData && c.userData.containsItem === 'fuel_cell' && c.userData.mesh && !c.userData.isDestroyed) {
                if (Math.random() < 0.35) {
                    const smokePos = c.userData.mesh.position.clone().add(new THREE.Vector3(0, 1.8, 0));
                    spawnSmokeParticle(smokePos);
                }
            }
        });
        ...
        for (let i = smokeParticles.length - 1; i >= 0; i--) {
            ...
            p.mesh.scale.setScalar(p.baseScale * (1 + progress * 2.5));
            if (p.mesh.material) {
    ...
    if (isActive) {
                ...
                if (aliens.some(a => a === obj || a.getObjectById(obj.id))) {
                    ...
                    xrayMatToApply = xrayMaterials.fuelCell;
                     ...
                     const isDynamic = bullets.includes(obj) ||
                                    ...
                                    smokeParticles.some(sp=>sp.mesh===obj) ||
        ...
        if (levelData.fogDensity) {
            scene.fog = new THREE.FogExp2(levelData.fogColor, levelData.fogDensity);
             ...
             const fogDistance = { city: 70, desert: 100, volcanic: 80, ice: 120, toxic: 80, crystal: 220 }[currentLevel] || 100;
...
checkBuildingCollision = function(objectBox, isProjectile = true) {
    if (currentLevel === 'city' && cityWindows.length > 0 && isProjectile) {
        ...
        for (let i = 0; i < cityWindows.length; i++) {
            ...
            if (win.center && win.center.distanceToSquared(objCenter) > 25.0) {
                continue;
            }

            const windowBox = win.expandedBox;

            if (windowBox && windowBox.intersectsBox(objectBox)) {
                breakWindow(win, objCenter);
                return true; 
            }
        }
    }
    return originalCheckBuildingCollision(objectBox, isProjectile);
};

function createAlienDebris(position, color) {
    const n = Math.floor(Math.random() * 16) + 30;
    const m = new THREE.MeshStandardMaterial({ color: color, emissive: color, emissiveIntensity: 2.5 });
    for (let i = 0; i < n; i++) {
        const d = new THREE.Mesh(sharedDebrisGeometry, m); 
...
function breakWindow(windowRef, hitPosition) {
    windowRef.isBroken = true;

    if (windowRef.isInstanced) {
        const instancedMesh = windowRef.mesh;
        const dummyMatrix = new THREE.Matrix4();
        dummyMatrix.makeScale(0, 0, 0);
        instancedMesh.setMatrixAt(windowRef.instanceId, dummyMatrix);
        instancedMesh.instanceMatrix.needsUpdate = true;

        const brokenGeo = new THREE.BoxGeometry(0.8, 1.6, 0.1);
        const brokenMat = new THREE.MeshStandardMaterial({
            color: (gameSettings.texturesEnabled && window.cityWindowBrokenTexture) ? 0xffffff : 0x111111,
            map: (gameSettings.texturesEnabled && window.cityWindowBrokenTexture) ? window.cityWindowBrokenTexture : null,
            roughness: 0.9
        });
        ...
        if (windowMesh) {
            ...
            if (Array.isArray(windowMesh.material)) {
                windowMesh.material = windowMesh.material.map(m => m.clone());
            } else if (windowMesh.material) {
                windowMesh.material = windowMesh.material.clone();
            }

            const mat = Array.isArray(windowMesh.material) ? windowMesh.material[0] : windowMesh.material;

            if (mat) {
                if (gameSettings.texturesEnabled && window.cityWindowBrokenTexture) {
                    mat.map = window.cityWindowBrokenTexture;
...
function createGlassShards(position) {
    ...
    for (let i = 0; i < count; i++) {
        const shard = new THREE.Mesh(sharedShardGeometry, sharedShardMaterial);
...
function updateGlassShards(delta) {
    for (let i = glassShards.length - 1; i >= 0; i--) {
        ...
        s.velocity.y -= GRAVITY * 0.5 * delta;
