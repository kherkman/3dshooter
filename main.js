...
function switchMusicToLevel(levelName) {
    ...
    // Fix: Removed the restrictive !isPaused condition.
    // This allows music to play instantly when loaded, avoiding asynchronous Pointer Lock glitches.
    if (hasInteracted) {
...
function rebuildPlayerGunModels() {
    ...
    // Re-create gun models (this will now successfully evaluate window.gunMetalTexture.image to true)
    GameData.weapons.forEach(w => gunModels.push(w.model(true)));
        ...
        if (isJetpackActive) {
            if (!jetpackSoundActive) {
                ...
                if (gameSounds['jetpack']) {
                    ...
                    jetpackSound.setLoop(false); // Soitetaan vain kerran liikkeelle lähdettäessä
