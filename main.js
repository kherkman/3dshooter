...
function init() {
    ...
    sharedDebrisGeometry = new THREE.SphereGeometry(0.06, 8, 8); // reusable tiny sphere geometry for enemy debris
...
function clearScene() {
    ...
    glassShards.forEach(s => {
        scene.remove(s.mesh);
        // Do not dispose of optimized shared geometry and material
    });
...
function loadLevel(levelName, isInitialLoad = false, isLanding = false) {
    ...
    // Dynamic city window identification and precomputed bounding box caching (O(1) frame lookup)
    cityWindows.length = 0;
    if (levelName === 'city') {
        scene.traverse(child => {
            if (child.isMesh && child.material && child.material.emissive && child.material.emissive.getHex() === 0xffaa00) {
                ...
                // Precompute the expanded bounding box once during level initialization
    ...
    if (currentLevel === 'city' && cityWindows.length > 0) {
        ...
        if (isSmallProjectile) {
            for (let i = 0; i < cityWindows.length; i++) {
                ...
                // Instant look up using cached, precomputed expanded bounding box
...
function createAlienDebris(position, color) {
    ...
    for (let i = 0; i < n; i++) {
        // Reuse optimized, pre-allocated sphere geometry instance
...
function createGlassShards(position) {
    const count = Math.floor(Math.random() * 11) + 12; // Optimized count (12 to 22 shards is plenty and keeps FPS fluid)

    for (let i = 0; i < count; i++) {
        // Reuse optimized shared geometry and material to prevent WebGL compile lag
