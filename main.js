...
let trashTexture, streetLightTexture; // New prop textures
...
function init() {
    ...
    // Load Prop Textures (Trash can & Street light)
    ...
    window.pistolBodyTexture = pistolBodyTexture;
...
function loadLevel(levelName, isInitialLoad = false, isLanding = false) {
    ...
    // CORRECTION: Spawn Spacecraft and Vehicles BEFORE Initial Collectibles!
    // This adds spacecraft.userData.colliderBox to buildingColliders prior to spawning fuel cells,
    // ensuring fuel cells and other items will NOT spawn inside the spacecraft's hitbox.
    ...
    // Dynamic city window identification and precomputed bounding box caching (Expanded for reliable hits)
    cityWindows.length = 0;
    if (levelName === 'city') {
        scene.traverse(child => {
            if (child.isMesh && child.material && child.material.emissive && child.material.emissive.getHex() === 0xffaa00) {
                ...
                box.expandByScalar(0.6); // Expanded to 0.6 to prevent fast projectile tunneling
...
function animate() {
    ...
    if (!isPaused) {
        ...
        // Stop hoverbike sound if game is paused
        if (hoverbikeSoundActive) {
            hoverbikeSoundActive = false;
            if (hoverbikeSound) {
                try { if (hoverbikeSound.isPlaying) hoverbikeSound.stop(); } catch(e) {}
