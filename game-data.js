const GameData = {
    ...
    items: {
        jetpack: {
            ...
            model: () => {
                const group = new THREE.Group();
                const hasJetpackTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.jetpackTexture;
                const bodyMaterial = new THREE.MeshStandardMaterial({ 
                    color: 0xffffff, 
                    map: hasJetpackTex ? window.jetpackTexture : null,
                    roughness: 0.3 
                });
                ...
                const body = new THREE.Mesh(new THREE.BoxGeometry(0.8, 1, 0.8), bodyMaterial);
                group.add(body);
                const thrusterL = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 1.2, 16), thrusterMaterial);
        ...
        health: {
            ...
            model: () => {
                ...
                const hasHealthTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.healthTexture;
                const packMaterial = new THREE.MeshStandardMaterial({ 
                    color: 0xffffff, 
                    map: hasHealthTex ? window.healthTexture : null,
                    emissive: hasHealthTex ? 0x000000 : 0xcccccc, 
                    emissiveIntensity: hasHealthTex ? 0.0 : 1.0 
                }); 
                const pack = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.5, 0.2), packMaterial); 
                group.add(pack); 
                const crossH = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.3, 0.22), redMaterial); 
                const crossV = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.1, 0.22), redMaterial); 
        ...
        fuel_cell: {
            ...
            model: () => {
                const group = new THREE.Group();
                const hasFuelCellTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.fuelCellTexture;
                const capsuleMat = new THREE.MeshStandardMaterial({ 
                    color: hasFuelCellTex ? 0xffffff : 0xbbbbbb, 
                    map: hasFuelCellTex ? window.fuelCellTexture : null,
                    metalness: hasFuelCellTex ? 0.3 : 0.9, 
                    roughness: 0.3 
                });
        ...
        glowing_orb: {
            ...
            model: () => {
                ...
                const hasOrbTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.glowingOrbTexture;
                const m = new THREE.MeshStandardMaterial({
                    color: hasOrbTex ? 0xffffff : 0x00ffff, 
                    map: hasOrbTex ? window.glowingOrbTexture : null,
                    emissive: hasOrbTex ? 0x000000 : 0x00ffff, 
                    emissiveIntensity: hasOrbTex ? 0.0 : 2, 
                    transparent: true, 
                    opacity: 0.8
                });
        ...
        xray_goggles: {
            ...
            model: () => {
                ...
                const hasGogglesTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.gogglesTexture;
                const lensMat = new THREE.MeshStandardMaterial({
                    color: hasGogglesTex ? 0xffffff : 0x111111, 
                    map: hasGogglesTex ? window.gogglesTexture : null,
                    metalness: hasGogglesTex ? 0.3 : 1.0, 
                    roughness: 0.1
                });
    ...
    vehicles: {
        motorcycle: {
            ...
            model: () => {
                const g = new THREE.Group();
                const hasHoverbikeTex = gameSettings.texturesEnabled && window.hoverbikeTexture;

                const bodyMat = new THREE.MeshStandardMaterial({
                    color: hasHoverbikeTex ? 0xffffff : 0xdd4444,
                    map: hasHoverbikeTex ? window.hoverbikeTexture : null,
                    metalness: 0.8,
                    roughness: 0.4
                });
