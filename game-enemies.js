...
function scheduleEnemyRespawn(delayMs = null) {
    // Asetettu pyydetty 9–20 sekunnin viive (9000 ms perusaika + 0–11000 ms lisäaika)
    if (delayMs === null) {
...
GameData.enemies = {
    ...
    dome_guardian: {
        ...
        levels: ['crystal'],
        spawnWeight: 0.1,
        properties: { health: 20, score: 150, spawnY: 4, sightRange: 80, attackRange: 70, attackCooldown: 3.5, attackDamage: 50 },
        model: () => {
            const group = new THREE.Group();
        ...
        animations: {
            ...
            scan: (alien, time, player, distance, playerDir, delta) => {
                ...
                // Jos pelaaja on näköetäisyydellä (80m), silmä kääntyy välittömästi pelaajaa kohti
                if (distance < props.sightRange) {
                    ...
                    eyePivot.quaternion.slerp(targetQuaternion, 0.12);

                    if (alien.userData.state === 'scanning') {
                        alien.userData.state = 'locked';
                        alien.userData.lockTimer = 0.8; // Salama latautuu 0.8 sekuntia ennen laukausta
                    }
                } else {
                    if (alien.userData.state === 'scanning') {
                        // Vapaa pyyhkäisyliike kun pelaaja ei ole lähellä
                        eyePivot.rotation.y = Math.sin(time * 0.4) * Math.PI * 0.4;
                        eyePivot.rotation.x = Math.sin(time * 0.25) * Math.PI * 0.2;
                    } else if (alien.userData.state === 'locked') {
                ...
                switch (alien.userData.state) {
                    ...
                    case 'locked':
                        eyeMaterial.emissiveIntensity = 3 + Math.sin(time * 30) * 2.5; // Kipinöivä hehku latauksen aikana
