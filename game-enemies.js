GameData.enemies = {
    ...
    cyborg: {
        ...
        model: () => {
            ...
            const hasEnemyBlackTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.enemyBlackTexture;
            const hasCyborgGrayTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.cyborgGrayTexture;
            const armorMat = new THREE.MeshStandardMaterial({ 
                color: hasCyborgGrayTex ? 0xffffff : 0x444444, 
                map: hasCyborgGrayTex ? window.cyborgGrayTexture : null,
                roughness: 0.25, 
                metalness: 0.9 
            }); 
...
// Optimized updateDebris to properly call .dispose() on dynamic pirstale-materiaalit to prevent WebGL memory leaks
function updateDebris(delta) { 
    for (let i = alienDebris.length - 1; i >= 0; i--) { 
        ...
        if (p.lifetime <= 0) { 
            scene.remove(p.mesh); 
            if (p.mesh.material) {
                p.mesh.material.dispose(); // Prevents memory leak of dynamic MeshStandardMaterials
            }
