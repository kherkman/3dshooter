...
// --- KESKITETTY VIHOLLISTEN ILMESTYMISVIIVE (RESPAWN DELAY) ---
function scheduleEnemyRespawn(delayMs = null) {
    if (delayMs === null) {
        delayMs = 9000 + Math.random() * 11000;
    }

    setTimeout(() => {
        if (!isGameOver && typeof spawnAliens === 'function') {
            spawnAliens(1);
        }
    }, delayMs);
}

// Altistetaan funktio globaalisti muille moduuleille
window.scheduleEnemyRespawn = scheduleEnemyRespawn;

GameData.enemies = {
    ground: {
        name: 'Bionic Bug',
        description: 'A fast, spider-like robot that chases, climbs walls, and pounces on its target.',
        levels: ['city', 'desert', 'volcanic', 'toxic', 'crystal'],
        spawnWeight: 0.65,
        initialSpawn: false,
        properties: { health: 2, score: 15, spawnY: 0.6, sightRange: 40, chaseSpeed: 8, pounceRange: 35, pounceCooldown: 2.0, attackCooldown: 0.0, pounceWindUp: 0.25, pounceForce: 30, pounceLift: 12, pounceDamage: 20 },
        model: () => {
    ...
    flyer: {
        ...
        spawnWeight: 0.05,
        properties: { health: 1, score: 10, spawnY: () => Math.random() * 20 + 15, radius: 1.3, sightRange: 50, attackCooldown: 3.0, projectileSpeed: 30, patrolRadius: 20 },
        model: () => {
    ...
    cyborg: {
        ...
        model: () => {
            ...
            const gunMetal = new THREE.MeshStandardMaterial({
                color: hasEnemyBlackTex ? 0xffffff : 0x28292d,
                map: hasEnemyBlackTex ? window.enemyBlackTexture : null,
                metalness: 0.90,
                roughness: 0.25
            });
    ...
    shard_roller: {
        ...
        model: () => {
            ...
            const hasShardRollerTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.shardRollerTexture;
    ...
    shard_mite: {
        ...
        model: () => {
            const group = new THREE.Group();
            const hasShardMiteTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.shardMiteTexture;
    ...
    predator: {
        ...
        model: () => {
            ...
            const hasStalkerMouthTex = (typeof gameSettings !== 'undefined' && gameSettings.texturesEnabled) && window.stalkerMouthTexture;
    ...
    dome_guardian: {
        ...
        levels: ['crystal'],
        spawnWeight: 0.1,
        properties: { health: 20, score: 150, spawnY: 4, height: 8.0, radius: 4.5, sightRange: 80, attackRange: 70, attackCooldown: 2.5, attackDamage: 50 },
        model: () => {
            const group = new THREE.Group();
        ...
        animations: {
            ...
            scan: (alien, time, player, distance, playerDir, delta) => {
                const { eyePivot, eyeMaterial } = alien.userData;
                ...
                if (distance < props.sightRange) {
                    ...
                    // Suunnataan eyePivot sulavasti kohti pelaajaa ilman matriisikorruptiota
                    const eyeWorldPos = new THREE.Vector3();
                    eyePivot.getWorldPosition(eyeWorldPos);

                    const dirToTarget = targetWorldPos.clone().sub(eyeWorldPos).normalize();
                    if (dirToTarget.lengthSq() > 0.0001) {
                        const targetQuat = new THREE.Quaternion();
                        const forward = new THREE.Vector3(0, 0, 1);
                        targetQuat.setFromUnitVectors(forward, dirToTarget);

                        if (alien.parent) {
                            const parentQuat = alien.getWorldQuaternion(new THREE.Quaternion()).invert();
                            targetQuat.premultiply(parentQuat);
                        }

                        eyePivot.quaternion.slerp(targetQuat, 0.15);
                    }

                    if (alien.userData.state === 'scanning') {
                        alien.userData.state = 'locked';
                        alien.userData.lockTimer = 0.5;
                    }
                } else {
                    if (alien.userData.state === 'scanning') {
                        eyePivot.rotation.y = Math.sin(time * 0.4) * Math.PI * 0.4;
                        eyePivot.rotation.x = Math.sin(time * 0.25) * Math.PI * 0.2;
                    } else if (alien.userData.state === 'locked' || alien.userData.state === 'cooldown') {
                ...
                switch (alien.userData.state) {
                    ...
                    case 'firing':
                        ...
                        alien.userData.cooldownTimer = 2.0;
                        break;
...
function handleEnemyCollision(alien, deltaPos) {
    ...
    // KORJUS: Välitetään 'false' isProjectile-parametrille, jotta vihollisen liike EI tuhoa Crystal Brain -konetta!
...
function getGroundYForAlien(alien, defaultSpawnY) {
    ...
    const alienX = alien.position.x;
    const alienZ = alien.position.z;
    const alienY = alien.position.y;
...
function spawnAliens(count, isInitialSpawn = false) {
    ...
    for (let i = 0; i < count; i++) {
        // Tornikentässä (crystal) sijoitetaan osa vihollisista eri kerroksiin - PAKOTETTU VAIN CRAWLER ('ground') -VIHOLLISIKSI
        if (currentLevel === 'crystal' && levelObjects.towerPos && Math.random() < 0.4) {
            const towerPos = levelObjects.towerPos;
            const floorY = levelObjects.towerFloors ? levelObjects.towerFloors[Math.floor(Math.random() * levelObjects.towerFloors.length)] : 15;
            ...
            alien.userData.attackCooldown = 0;
            alien.userData.health = crawlerData.properties.health;

            if (crawlerData.animations) {
                crawlerData.animations.idle?.(alien, 0);
            }
        ...
        if (spawnSafe(alien, spawnY)) {
            if (selectedEnemy.type === 'dome_guardian') {
                ...
                alien.userData.attackCooldown = 0; // Alustetaan 0:aan hyppimisen/hyökkäyksen varmistamiseksi
            }
...
function updateAliens(delta) {
    ...
    for (let i = aliens.length - 1; i >= 0; i--) {
        ...
        // Vihollisten välinen työntövoima
        for (let j = i - 1; j >= 0; j--) {
        ...
        // --- PELAAJAN JA VIHOLLISTEN TÖRMÄYS (ESTETÄÄN VIHOLLISEN SISÄÄN MENO SEINÄTURVALLISESTI) ---
        if (playerObject && (player.state === 'on_foot' || player.state === 'driving_motorcycle')) {
            ...
            else if (data.type === 'ground') enemyRadius = 1.1;
            else if (data.type === 'dome_guardian') enemyRadius = 4.5;
            else if (data.type === 'predator') enemyRadius = 1.6;
            else if (data.type === 'shard_roller') enemyRadius = 1.8;
            else if (data.type === 'tentacle') enemyRadius = 1.4;
            else if (data.type === 'stingray') enemyRadius = 4.0;
            else if (data.type === 'worm_swarm') enemyRadius = 1.0;
            else if (data.type === 'flyer') enemyRadius = 1.3;

            const playerRadius = 0.7;
            const minDistance = enemyRadius + playerRadius;

            if (data.type === 'flyer') {
                // Täysi 3D-törmäystarkistus lentävälle Wasp-viholliselle
                const playerCenter = playerObject.position.clone().add(new THREE.Vector3(0, 0.9, 0));
                const dist3D = playerCenter.distanceTo(alien.position);
                if (dist3D < minDistance && dist3D > 0.0001) {
                    const overlap = minDistance - dist3D;
                    const pushVec = playerCenter.clone().sub(alien.position).normalize().multiplyScalar(overlap);
                    
                    if (typeof window.pushPlayerSafely === 'function') {
                        window.pushPlayerSafely(pushVec.x, pushVec.z);
                        playerObject.position.y += pushVec.y * 0.5;
                    } else {
                        playerObject.position.add(pushVec);
                    }
                }
            } else {
                const dx = playerObject.position.x - alien.position.x;
                const dz = playerObject.position.z - alien.position.z;
                const distXZ = Math.sqrt(dx * dx + dz * dz);

                const enemyHeight = (enemyProps.height || 2.0);
                const yOverlap = Math.abs((playerObject.position.y + 0.9) - alien.position.y) < (enemyHeight + 1.8) / 2;

                if (distXZ < minDistance && yOverlap && distXZ > 0.0001) {
                    const overlap = minDistance - distXZ;
                    const pushX = (dx / distXZ) * overlap;
                    const pushZ = (dz / distXZ) * overlap;

                    // TÖNÄISTÄÄN PELAAJAA SEINÄTURVALLISESTI (Ei mene seinän läpi)
                    if (typeof window.pushPlayerSafely === 'function') {
                        window.pushPlayerSafely(pushX, pushZ);
                    } else {
        ...
        switch(data.type) {
            ...
            case 'ground': {
                const targetGroundY = getGroundYForAlien(alien, enemyProps.spawnY);
                const distanceToPlayer = alien.position.distanceTo(playerTarget.position);
                const enemyDefAnims = GameData.enemies.ground.animations;

                // Seinän tunnistus crawlerin edessä kiipelyä ja pystykääntymistä varten
                const forwardDir = new THREE.Vector3(0, 0, -1).applyQuaternion(alien.quaternion);
                const wallCheckPos = alien.position.clone().add(forwardDir.clone().multiplyScalar(1.0));
                wallCheckPos.y += 0.5;
                ...
                // Animoidaan pystykääntyminen sulavasti
                if (enemyDefAnims && enemyDefAnims.updateClimbPitch) {
                    enemyDefAnims.updateClimbPitch(alien, isClimbing, delta);
                }

                const onGround = (!isClimbing && (alien.position.y <= targetGroundY + 0.25 || Math.abs(data.velocity.y) < 0.1));

                switch (data.state) {
                    case 'idle': 
                        if (distanceToPlayer < enemyProps.sightRange) { data.state = 'chase'; } 
                        if (enemyDefAnims) enemyDefAnims.idle(alien, clock.getElapsedTime()); 
                        data.velocity.x *= 0.8; 
                        data.velocity.z *= 0.8; 
                        break;
                    case 'chase': 
                        alien.lookAt(playerTarget.position.x, alien.position.y, playerTarget.position.z); 
                        const dirToPlayer = new THREE.Vector3().subVectors(playerTarget.position, alien.position).normalize(); 

                        if (isClimbing) {
                            // Kiivetään seinää ylöspäin
                            data.velocity.x = dirToPlayer.x * enemyProps.chaseSpeed * 0.5;
                            data.velocity.z = dirToPlayer.z * enemyProps.chaseSpeed * 0.5;
                            data.velocity.y = enemyProps.chaseSpeed * 0.9;
                        } else {
                            data.velocity.lerp(dirToPlayer.multiplyScalar(enemyProps.chaseSpeed), 0.1); 
                        }

                        if (enemyDefAnims) enemyDefAnims.walk(alien, clock.getElapsedTime()); 

                        // Varmistettu hyppyhyökkäyksen laukaisu
                        const canAttack = (data.attackCooldown === undefined || data.attackCooldown <= 0);
                        if (distanceToPlayer < enemyProps.pounceRange && canAttack && onGround) { 
                            data.state = 'pounce'; 
                            data.pounceTimer = enemyProps.pounceWindUp; 
                            data.velocity.set(0, 0, 0); 
                        } 
                        break;
                    case 'pounce': 
                        if (enemyDefAnims) enemyDefAnims.pounce(alien); 
                        alien.lookAt(playerTarget.position.x, alien.position.y, playerTarget.position.z); 
                        data.pounceTimer -= delta; 
                        if (data.pounceTimer <= 0) { 
                            const pounceDir = new THREE.Vector3().subVectors(playerTarget.position, alien.position); 
                            pounceDir.y = 0; 
                            pounceDir.normalize().multiplyScalar(enemyProps.pounceForce); 
                            pounceDir.y = enemyProps.pounceLift; 
                            data.velocity.copy(pounceDir); 
                            data.state = 'leaping'; 
                            data.attackCooldown = enemyProps.pounceCooldown; 
                        } 
                        break;
                    case 'leaping': 
                        if (enemyDefAnims) enemyDefAnims.leap(alien); 
                        if ((player.state === 'on_foot' || player.state === 'driving_motorcycle') && distanceToPlayer < 1.8) { 
                            health = Math.max(0, health - enemyProps.pounceDamage); 
                            lastAttackerPosition = alien.position.clone(); 
                            playSound('player_damage'); 

                            // HYPPYISKU/KNOCKBACK PELAAJAAN: Annettaan pelaajalle fysiikkainpulssi
                            const knockbackDir = new THREE.Vector3().subVectors(playerTarget.position, alien.position);
                            knockbackDir.y = 0;
                            knockbackDir.normalize();
                            if (player.velocity) {
                                player.velocity.x += knockbackDir.x * 22.0;
                                player.velocity.z += knockbackDir.z * 22.0;
                                player.velocity.y += 8.0; // Heittää pelaajaa ilmaan
                            }
                            if (typeof window.pushPlayerSafely === 'function') {
                                window.pushPlayerSafely(knockbackDir.x * 0.8, knockbackDir.z * 0.8);
                            }

                            data.velocity.multiplyScalar(-0.5); 
                            data.state = 'idle'; 
                        } 
                        if (onGround && data.velocity.y < 0) { 
                            data.state = 'idle'; 
                        } 
                        break;
                }
        ...
        if (data.type !== 'dome_guardian') {
            ...
            if (data.type === 'ground' || data.type === 'worm_swarm' || data.type === 'shard_roller' || data.type === 'shard_mite' || data.type === 'predator') {
                const targetGroundY = getGroundYForAlien(alien, enemyProps.spawnY);
                if (alien.position.y < targetGroundY) {
                    alien.position.y = targetGroundY;
