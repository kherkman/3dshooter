 ...
 * Manages UI menu toggling logic and vertical/horizontal physics collisions.
 */
...
const playerCeilingRaycaster = new THREE.Raycaster();
const playerCeilingRayDirection = new THREE.Vector3(0, 1, 0);
...
function setupControls() {
    ...
    const sbsImageOffsetSlider = document.getElementById('sbs-image-offset');
    if (sbsImageOffsetSlider) {
        sbsImageOffsetSlider.value = gameSettings.sbsImageOffset || 0;
        sbsImageOffsetSlider.addEventListener('input', (e) => {
            gameSettings.sbsImageOffset = parseFloat(e.target.value);
        });
    }
...
function handleGamepadOptionsNavigation(currentGamepad, delta) {
    ...
    // Jos valittuna on liukukytkin (Volume/Separation/Offset slider)
...
function updatePlayer(delta) {
        ...
        for (const c of buildingColliders) {
            ...
            if (c.userData && c.userData.colliderType === 'mesh') {
                ...
                if (ceilingHits.length > 0) {
                    const ceilingY = ceilingHits[0].point.y;
                    const playerHeadY = playerObject.position.y + playerHeight / 2;
                    if (playerHeadY >= ceilingY - 0.3 && playerHeadY <= ceilingY + 0.8) {
                        player.velocity.y = 0;
                        playerObject.position.y = ceilingY - playerHeight / 2 - 0.05;
                        deltaPos.y = 0;
                        break;
                    }
                }
    ...
    playerFloorRayOrigin.y += playerHeight / 2 - 0.05;
    ...
    for (const c of buildingColliders) {
        ...
        if (c.userData && c.userData.colliderType === 'mesh') {
            ...
            if (intersections.length > 0) {
                ...
                if (hitY <= playerObject.position.y + 0.3) {
