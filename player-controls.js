...
function updatePlayer(delta) {
    ...
    // Optimized: Set up the pre-allocated shared raycaster variables instead of per-frame instantiations
...
function updatePlayerVehicle(delta) {
    ...
    // Movement is based on W/S keys or touch controls
    ...
    // --- Touchscreen Hoverbike Movement & Steering (Left Side) ---
    for (const id in touchState) {
        const state = touchState[id];
        if (state.zone === 'move') {
            const dx = state.currentX - state.startX;
            ...
            if (Math.abs(dy) > deadZone) {
                // Drag up (negative dy) = accelerate forward, Drag down (positive dy) = reverse
            ...
            if (Math.abs(dx) > deadZone) {
                // Drag right (positive dx) = turn right (-Y rot), Drag left (negative dx) = turn left (+Y rot)
    ...
    // Optimized: Mathematical Box3 bounding box (size approx 1.0 x 1.5 x 3.2 based on motorcycle meshes)
    ...
    // Optimized: Reuses and offsets the computed Box3 mathematically instead of calling the heavy setFromObject() again
