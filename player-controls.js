...
// --- OPTIMIZED SHARED COLLISION OBJECTS ---
const playerFloorRaycaster = new THREE.Raycaster();
const playerFloorRayOrigin = new THREE.Vector3();
const playerFloorRayDirection = new THREE.Vector3(0, -1, 0);
...
function setupControls() {
    ...
    const onMouseMove = (event) => {
        ...
        if (player.state === 'on_foot') {
            ...
            motorcycle.rotation.y -= movementX * 0.002;
            camera.rotation.x -= movementY * 0.002;
            camera.rotation.x = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, camera.rotation.x));
        }
    };
    const onKeyDown = (event) => {
        ...
        // ESC / Options button press logic: Close active modals first before options
        if (event.code === 'KeyO' || event.key === 'Escape') { 
            if (debugMenu && debugMenu.style && debugMenu.style.display.includes('flex')) {
                toggleDebugMenu();
                return;
            }
            if (inventoryMenu && inventoryMenu.style && inventoryMenu.style.display.includes('flex')) {
                toggleInventoryMenu();
                return;
            }
            toggleOptionsMenu(); 
    ...
    if (musicSlider) {
        ...
        musicSlider.addEventListener('input', (e) => { gameSettings.musicVolume = parseFloat(e.target.value); if(backgroundMusic) backgroundMusic.volume = gameSettings.musicVolume; });
    }
    ...
    const onTouchStart = (event) => {
        ...
        // Allow scrolling inside open menu overlays without triggering touch actions
        if (event.target.closest('.menu-box, .overlay')) return;
    ...
    const onTouchMove = (event) => {
        ...
        // Allow native touch scrolling inside menu boxes
        if (event.target.closest('.menu-box, .overlay')) return;
    ...
    if (showGamepadControlsBtnOptions) {
        ...
        showGamepadControlsBtnOptions.addEventListener('click', showGamepadControls);
        showGamepadControlsBtnOptions.addEventListener('touchend', showGamepadControls);
    }

    const closeGamepadControlsBtn = document.getElementById('close-gamepad-controls');
    if (closeGamepadControlsBtn) {
        const closeGamepadControls = (e) => {
            if (e) e.preventDefault();
            const gamepadMenu = document.getElementById('gamepad-controls-menu');
            if (gamepadMenu) gamepadMenu.style.display = 'none';
        };
        closeGamepadControlsBtn.addEventListener('click', closeGamepadControls);
        closeGamepadControlsBtn.addEventListener('touchend', closeGamepadControls);
    }
...
function handleGamepadInventoryNavigation(currentGamepad, delta) {
    ...
    if (gamepadInvDebounceTimer <= 0) {
        if (dpadDown || stickY > 0.5) {
            gamepadInvSelectedIndex = (gamepadInvSelectedIndex + 1) % items.length;
            gamepadInvDebounceTimer = 0.2;
        } else if (dpadUp || stickY < -0.5) {
            gamepadInvSelectedIndex = (gamepadInvSelectedIndex - 1 + items.length) % items.length;
            gamepadInvDebounceTimer = 0.2;
        }
    }

    items.forEach((item, idx) => {
        if (idx === gamepadInvSelectedIndex) {
            item.style.border = '2px solid #00ffff';
            item.style.backgroundColor = 'rgba(0, 255, 255, 0.25)';
            item.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
        } else {
            item.style.border = '1px solid #ff8800';
            item.style.backgroundColor = 'rgba(0,0,0,0.3)';
        }
    });

    // Select with Button 0 (A / Cross)
...
function updateGamepadControls(delta) {
    ...
    const isJustPressed = (btnIdx) => {
        const pressed = currentGamepad.buttons[btnIdx]?.pressed || false;
        const wasPressed = gamepadPrevButtons[btnIdx] || false;
        return pressed && !wasPressed;
    };

    // UI overlays state check
    const gamepadMenu = document.getElementById('gamepad-controls-menu');
    const touchMenu = document.getElementById('touch-controls-menu');
    const optionsMenu = document.getElementById('options-menu');
    ...
    const isGamepadMenuOpen = gamepadMenu && gamepadMenu.style.display.includes('flex');
    const isTouchMenuOpen = touchMenu && touchMenu.style.display.includes('flex');
    const isOptionsMenuOpen = optionsMenu && optionsMenu.style.display.includes('flex');
    ...
    // Dynamic Gamepad Inventory Navigation
    if (isInvMenuOpen) {
        handleGamepadInventoryNavigation(currentGamepad, delta);
    ...
    // 2. Kolmionappi (Button 3: Triangle / Y) -> Avaa/sulkee INV-ikkunan
    if (isJustPressed(3)) {
        toggleInventoryMenu();
    }

    // 3. Neliönappi (Button 2: Square / X) -> Avaa/sulkee Kartan
    if (isJustPressed(2)) {
        if (mapContainer) {
    ...
    // Start / Options Button (Button 9) -> Toggle Options Menu
    if (isJustPressed(9)) {
        toggleOptionsMenu();
    }
    ...
    // 4. CAMERA LOOK (Katseen kääntäminen nopeutettu telta-pohjaisella kertoimella 3.5)
    const lookX = currentGamepad.axes[2];
    const lookY = currentGamepad.axes[3];
    const lookSensitivity = 3.5;

    if (player.state === 'on_foot') {
        if (Math.abs(lookX) > deadzone) {
            playerObject.rotation.y -= lookX * lookSensitivity * delta;
        }
        if (Math.abs(lookY) > deadzone) {
            camera.rotation.x -= lookY * lookSensitivity * delta;
            camera.rotation.x = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, camera.rotation.x));
        }
    } else if (player.state === 'driving_motorcycle' && motorcycle) {
        if (Math.abs(lookX) > deadzone) {
            motorcycle.rotation.y -= lookX * lookSensitivity * delta;
        }
        if (Math.abs(lookY) > deadzone) {
            camera.rotation.x -= lookY * lookSensitivity * delta;
            camera.rotation.x = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, camera.rotation.x));
    ...
    // Jump / Jetpack (Button 0: A / Cross)
    keys['Space'] = currentGamepad.buttons[0]?.pressed || false;

    // Shoot (Button 7: RT / R2)
    ...
    // 6. Vuorovaikutus / Interact (Button 6: LT / L2) -> Launch Spacecraft, Drive Hoverbike, Collect/Throw Orb
    if (isJustPressed(6)) {
        if (player.state === 'driving_motorcycle') {
            exitMotorcycle();
        } else {
            keys['KeyF'] = true;
            fKeyPressed = true;
    ...
    // Switch Weapons (LB / RB - Buttons 4 & 5)
    if (isJustPressed(4)) changeWeapon(-1);
    if (isJustPressed(5)) changeWeapon(1);

    // Record previous button states for edge triggering
    for (let i = 0; i < currentGamepad.buttons.length; i++) {
        gamepadPrevButtons[i] = currentGamepad.buttons[i]?.pressed || false;
    }
}
...
function updatePlayer(delta) {
    ...
    playerFloorRayOrigin.copy(playerObject.position);
    playerFloorRayOrigin.y += 0.1;
    ...
    for (const id in touchState) {
        ...
        if (state.zone === 'move') {
            ...
            if (Math.abs(dy) > deadZone) {
                const forwardFactor = Math.min(Math.max(-dy / maxDist, -1.0), 1.0);
                if (forwardFactor > 0) {
                    bikeData.velocity.add(bikeForward.clone().multiplyScalar(bikeProps.speed * forwardFactor * delta));
                } else {
                    bikeData.velocity.add(bikeForward.clone().multiplyScalar(bikeProps.speed * 0.5 * forwardFactor * delta));
                }
            }

            if (Math.abs(dx) > deadZone) {
                const turnFactor = Math.min(Math.max(dx / maxDist, -1.0), 1.0);
                motorcycle.rotation.y -= turnFactor * lookSpeed * 1.2 * delta;
            }
        }
    }
...
function enterMotorcycle() {
    ...
    gunModels.forEach(g => { if (g) g.visible = false; });
    const index = interactables.splice(interactables.findIndex(i => i.mesh === motorcycle), 1);
